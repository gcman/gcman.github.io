...
RUNS = 10
ROOT = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))))
...
def save_data():
    with open(os.path.join(EULER_DATA_DIR,"problemData.json"),"w",encoding="utf-8") as f:
        dump(DATA, f)
...
def compute_time(n,runs):
    path = os.path.join(ROOT,"scripts/euler/euler-solutions/") + DATA[str(n)]["path"] + "/main.py"
