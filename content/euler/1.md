...
description = "My Python solution and code for Project Euler 1 on Project Euler."
date = 2018-06-09
tags = ["identity"]
...
The sum of the natural numbers up to \\(n\\) is the \\(n\\)-th [triangular
number](https://en.wikipedia.org/wiki/Triangular%5Fnumber). Let's call this \\(T(n)\\). A well known formula for this is
...
\\(i\\) is divisible by 3 _or_ 5 would have time complexity \\(O(n)\\). With the large
